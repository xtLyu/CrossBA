...
from attack.attack_data import clean_pretrain_class_dataset, multi_class_NIG_clean_class, multi_class_NIG_poison_class, \
    induced_graphs_nodes_poison_class, load_tasks_class, load_backdoor_task_class
...
def data_process(dataname, num_trigger_node, pre_class_list, down_class_list, trigger_pattern):
    data = pk.load(open('./Dataset/class/{}/feature_reduced_pretrain.data'.format(dataname), 'br'))
    induced_graphs_nodes_poison_class(data, dataname=dataname, class_list=pre_class_list, smallest_size=100,
    ...
    induced_graphs_nodes_poison_class(data, dataname=dataname, class_list=down_class_list, smallest_size=100,
                                      largest_size=300, num_trigger_nodes=num_trigger_node,
                                      trigger_pattern=trigger_pattern)
...
def prompt_w_h_poison(backdoor_node_features, makeTE, target_embedding, device, num_trigger_node, pretext, folder_path,
    ...
    _, _, train_list, test_list = multi_class_NIG_clean_class(dataname, down_class_list, shots=200)
    ...
    poison_train_list, poison_test_list = multi_class_NIG_poison_class(dataname, down_class_list, shots=200)
...
def meta_train_maml(epoch, maml, gnn, lossfn, opt, meta_train_task_id_list, dataname, adapt_steps=2, K_shot=100):
    ...
    for ep in range(epoch):
        task_gradients = []
        for task_1, task_2, support, query, total_num in load_tasks_class('train', task_pairs, dataname, K_shot, seed):
...
def meta_test_adam(is_poison, meta_test_task_id_list, dataname, K_shot, seed, maml, gnn, adapt_steps_meta_test, lossfn,
    ...
    for task_1, task_2, support, query, _ in load_tasks_class('test', task_pairs, dataname, K_shot, seed):
        ...
        task_1_b, task_2_b, query_b, _ = next(load_backdoor_task_class('test', task_pairs, dataname, K_shot, seed))
...
if __name__ == '__main__':
    ...
    folder_path = f'./results/class/{args.dataset}_{current_time}_{args.gnn_type}_{args.usage_par}_{args.defense_mode}_{args.sim_thre}_{args.attack_type}_{args.makeTE}_{args.poisoned_node}_{args.reg_param_trigger_n}_{args.reg_param_trigger_f}_{args.reg_param_gnn}'
    folder_paths = f'./results/class/{args.dataset}_{current_time}_{args.gnn_type}_{args.usage_par}_{args.defense_mode}_{args.sim_thre}_{args.attack_type}_{args.makeTE}_{args.poisoned_node}_{args.reg_param_trigger_n}_{args.reg_param_trigger_f}_{args.reg_param_gnn}/pic_result'
