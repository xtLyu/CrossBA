...
def our_badpretrain(device, num_class, reg_param_gnn, folder_path, num_trigger_node, makeTE, gnn_type, dataname,
    ...
    graph_list, input_dim, hid_dim = attack_data.load_data4pretrain_class(dataname)
    ...
    if makeTE == 'cluster_random':
        cluster_centers_list = attack_pretrain.make_target_embedding(graph_list, pt.model, len(pretrain_class_list),
                                                                     device)
    ...
    optimizer_gnn = optim.Adam(poisoned_pre_model_gnn.parameters(), lr=0.001)
...
def prompt_w_h_poison(backdoor_node_features, makeTE, target_embedding, device, num_trigger_node, folder_path,
                      ...
                      defense_mode="none", sim_thre=0):
    logger.info("load data on classes:{}...".format(down_class_list))
    _, _, train_list, test_list = attack_data.multi_class_NIG_clean_class(dataname, down_class_list, shots=100)
    ...
    poison_train_list, poison_test_list = attack_data.multi_class_NIG_poison_class(dataname, down_class_list, shots=100)
    ...
    gnn, PG_aggregator, opi_pg, lossfn = model_create(is_poison=is_poison, dataname=dataname, gnn_type=gnn_type,
...
if __name__ == '__main__':
    ...
    prompt_w_h_poison(backdoor_node_features=backdoor_node_feature, makeTE=args.makeTE,
                      ...
                      dataname=args.dataset, gnn_type=args.gnn_type, num_class=num_class, is_poison=True,
