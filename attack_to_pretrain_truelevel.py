...
from attack.attack_data import induced_graphs_graphdataset, multi_class_NIG_poison, multi_class_NIG_clean, \
    load_data4pretrain_truegraph, load_tasks, load_backdoor_task
...
def data_process(dataname, num_trigger_node, node_classes, trigger_pattern):
    data = pk.load(open('./Dataset/{}/feature_increased.data'.format(dataname), 'br'))
    induced_graphs_graphdataset(data, dataname=dataname, num_classes=node_classes, num_trigger_nodes=num_trigger_node,
...
def our_badpretrain(device, num_class, reg_param_gnn, folder_path, num_trigger_node, makeTE, pretext,
    ...
    graph_list, input_dim, hid_dim = load_data4pretrain_truegraph(dataname, num_parts)
    ...
    for epoch in range(1, 10 + 1):
        for inter_epoch in range(1, 15 + 1):
            total_trigger_loss, total_lp, total_ln, total_lf = attack_pretrain.optimize_trigger(
...
if __name__ == '__main__':
    ...
    parser.add_argument("--predataset", type=str, default="ENZYMES")
    parser.add_argument("--downdataset", type=str, default="CiteSeer",
                        choices=['Computers', 'PubMed', 'Cora', 'CiteSeer', 'Photo'])
    ...
    folder_path = f'./results/level/{args.predataset}_{args.downdataset}_{current_time}_{args.gnn_type}_{args.usage_par}_{args.defense_mode}_{args.sim_thre}_{args.attack_type}_{args.makeTE}_{args.poisoned_node}_{args.reg_param_trigger_n}_{args.reg_param_trigger_f}_{args.reg_param_gnn}'
    folder_paths = f'./results/level/{args.predataset}_{args.downdataset}_{current_time}_{args.gnn_type}_{args.usage_par}_{args.defense_mode}_{args.sim_thre}_{args.attack_type}_{args.makeTE}_{args.poisoned_node}_{args.reg_param_trigger_n}_{args.reg_param_trigger_f}_{args.reg_param_gnn}/pic_result'
    ...
    if args.predataset == "ENZYMES":
        pre_num_class = 6
    else:
        raise ValueError

    if args.downdataset == "CiteSeer":
        down_num_class = 6
        adapt_steps = 20
        down_meta_train_task_id_list = [0, 1, 2, 3]
        down_meta_test_task_id_list = [4, 5]

    elif args.downdataset == "Cora":
        down_num_class = 7
        adapt_steps = 20
    ...
    if args.usage_par == 'prompt':
        ...
        meta_test_adam(is_poison=True, meta_test_task_id_list=down_meta_test_task_id_list, dataname=args.downdataset,
                       K_shot=200, seed=seed, maml=backdoor_maml, gnn=backdoor_gnn, adapt_steps_meta_test=adapt_steps,
