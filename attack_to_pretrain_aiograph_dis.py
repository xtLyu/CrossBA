...
from attack.attack_data import multi_class_NIG_poison_aiolevel, multi_class_NIG_clean_aiolevel, \
    load_data4pretrain, load_tasks_aiolevel, load_backdoor_task_aiolevel, induced_graphs_aio_graphs_poison
...
def prompt_w_h_poison(backdoor_node_features, makeTE, target_embedding, device, num_trigger_node, pretext, folder_path,
                      dataname="CiteSeer", gnn_type="TransformerConv", num_class=6, is_poison=True,
                      task_type='multi_class_classification', defense_mode="none", sim_thre=0):
    _, _, train_list, test_list = multi_class_NIG_clean_aiolevel(dataname, num_class, shots=200)
    ...
    poison_train_list, poison_test_list = multi_class_NIG_poison_aiolevel(dataname, num_class, shots=200)
    ...
    gnn, PG, opi_pg, lossfn, answering, opi_answer = model_create(is_poison, dataname, gnn_type, num_class, folder_path,
                                                                  pretext, task_type, True)
    answering.to(device)
    ...
    for i in range(1, outer_epoch + 1):
        ...
        PG.eval()
        gnn.eval()
        main_acc = acc_f1_over_batches(test_loader, PG, gnn, answering, num_class, task_type, device=device)
        ...
        PG.eval()
        gnn.eval()
        asr = asr_f1_over_batches(poison_test_loader1, PG, gnn, answering, num_class, task_type, device=device)
        asr_list.append(asr)
...
def meta_test_adam(is_poison, meta_test_task_id_list, dataname, K_shot, seed, maml, gnn, adapt_steps_meta_test, lossfn,
    ...
    for task_1, task_2, support, query, _ in load_tasks_aiolevel('test', task_pairs, dataname, K_shot, seed):
        ...
        logger.info("trigger node feature is {}".format(backdoor_node_features))

        if defense_mode == "prune_e":
            preprocess.prune_edges_by_similarity(query_b, sim_thre, device)
        elif defense_mode == "prune_ne":
            preprocess.prune_edges_and_nodes_by_similarity(query_b, sim_thre, device)
        elif defense_mode == "prune_graph":
            preprocess.prune_edges_by_similarity_graph(query_b, sim_thre, device, num_trigger_node)
        else:
            logger.info("no defense")
...
if __name__ == '__main__':
    ...
    parser.add_argument("--dataset", type=str, default="CiteSeer",
                        choices=['Computers', 'PubMed', 'Cora', 'Flickr', 'CiteSeer', 'Photo'])
    ...
    folder_path = f'./results/aiolevel/{args.dataset}_{current_time}_{args.gnn_type}_{args.usage_par}_{args.defense_mode}_{args.sim_thre}_{args.attack_type}_{args.makeTE}_{args.poisoned_node}_{args.reg_param_trigger_n}_{args.reg_param_trigger_f}_{args.reg_param_gnn}'
    folder_paths = f'./results/aiolevel/{args.dataset}_{current_time}_{args.gnn_type}_{args.usage_par}_{args.defense_mode}_{args.sim_thre}_{args.attack_type}_{args.makeTE}_{args.poisoned_node}_{args.reg_param_trigger_n}_{args.reg_param_trigger_f}_{args.reg_param_gnn}/pic_result'
    ...
    if args.dataset == "CiteSeer":
        ...
        num_class = 7
        adapt_steps = 5
        down_meta_train_task_id_list = [0, 1, 2, 3, 4]
        down_meta_test_task_id_list = [5, 6]
    ...
    if args.usage_par == 'prompt':
        ...
        train_meta_epoch = 30
        adapt_steps_meta_test_epoch = 10
        if args.dataset == 'PubMed' and args.gnn_type == 'GAT':
            train_meta_epoch = 50
            adapt_steps_meta_test_epoch = 100

        if args.dataset == 'Cora':
            adapt_steps_meta_test_epoch = 20
        ...
        meta_train_maml(train_meta_epoch, backdoor_maml, backdoor_gnn, backdoor_lossfn, backdoor_opt,
                        ...
                        args.dataset, adapt_steps=adapt_steps, K_shot=200)
        ...
        meta_test_adam(is_poison=True, meta_test_task_id_list=down_meta_test_task_id_list, dataname=args.dataset,
                       ...
                       adapt_steps_meta_test=adapt_steps_meta_test_epoch, lossfn=backdoor_lossfn,
