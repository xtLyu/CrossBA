...
from attack.attack_data import multi_class_NIG_poison_aiolevel, multi_class_NIG_clean_aiolevel, load_data4pretrain, \
    load_tasks_aiolevel, load_backdoor_task_aiolevel, induced_graphs_aio_graphs_poison
...
def data_process(dataname, num_trigger_node, node_classes, trigger_pattern):
    data = pk.load(open('./Dataset/{}/feature_reduced.data'.format(dataname), 'br'))
    induced_graphs_aio_graphs_poison(data, dataname=dataname, num_classes=node_classes, smallest_size=100,
...
def our_badpretrain(device, num_class, reg_param_gnn, folder_path, num_trigger_node, makeTE, pretext,
    ...
    if makeTE == 'cluster_random':
        cluster_centers_list = attack_pretrain.make_target_embedding(graph_list, pt.model.gnn, num_class, device)
...
def model_create(is_poison, pre_dataname, gnn_type, down_num_class, folder_path, pretext,
                 task_type='multi_class_classification', tune_answer=False):
    if task_type in ['multi_class_classification', 'regression']:
        ...
        if tune_answer:
            ...
            PG = LightPrompt(token_dim=input_dim, token_num_per_group=tnpc, group_num=down_num_class, inner_prune=0.01)
        ...
        if tune_answer:
            if task_type == 'regression':
                answering = torch.nn.Sequential(torch.nn.Linear(hid_dim, down_num_class), torch.nn.Sigmoid())
            else:
                answering = torch.nn.Sequential(torch.nn.Linear(hid_dim, down_num_class), torch.nn.Softmax(dim=1))
...
def prompt_w_h_poison(backdoor_node_features, makeTE, target_embedding, device, num_trigger_node, pretext, folder_path,
                      ...
                      down_num_class=7, is_poison=True, task_type='multi_class_classification', defense_mode="none",
                      sim_thre=0):
    _, _, train_list, test_list = multi_class_NIG_clean_aiolevel(down_dataname, down_num_class, shots=200)
    ...
    poison_train_list, poison_test_list = multi_class_NIG_poison_aiolevel(down_dataname, down_num_class,
                                                                          shots=200)
    ...
    gnn, PG, opi_pg, lossfn, answering, opi_answer = model_create(is_poison, pre_dataname, gnn_type, down_num_class,
    ...
    for i in range(1, outer_epoch + 1):
        ...
        PG.eval()
        gnn.eval()
        main_acc = acc_f1_over_batches(test_loader, PG, gnn, answering, down_num_class, task_type, device=device)
        ...
        PG.eval()
        gnn.eval()
        asr = asr_f1_over_batches(poison_test_loader1, PG, gnn, answering, down_num_class, task_type, device=device)
        asr_list.append(asr)
...
def model_components(device, is_poison, folder_path, pretext, down_dataname, dataname="CiteSeer",
...
def meta_train_maml(epoch, maml, gnn, lossfn, opt, meta_train_task_id_list, dataname, adapt_steps=2, K_shot=100):
    ...
    for ep in range(epoch):
        task_gradients = []
        for task_1, task_2, support, query, total_num in load_tasks_aiolevel('train', task_pairs, dataname, K_shot,
                                                                             seed):
...
def meta_test_adam(is_poison, meta_test_task_id_list, dataname, K_shot, seed, maml, gnn, adapt_steps_meta_test, lossfn,
    ...
    for task_1, task_2, support, query, _ in load_tasks_aiolevel('test', task_pairs, dataname, K_shot, seed):
        ...
        task_1_b, task_2_b, query_b, _ = next(load_backdoor_task_aiolevel('test', task_pairs, dataname, K_shot, seed))
        ...
        for _ in range(adapt_steps_meta_test):
            ...
            logger.info('{}/{} training loss: {:.8f}'.format(_, adapt_steps_meta_test, running_loss))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--predataset", type=str, default="Computers",
                        choices=['CiteSeer', 'Computers', 'PubMed', 'Cora', 'Computers', 'Photo'])
    ...
    parser.add_argument("--reg_param_trigger_n", type=float, default=0.01)
    parser.add_argument("--reg_param_trigger_f", type=float, default=0.01)
    ...
    folder_path = f'./results/aiolevel_dataset/{args.predataset}_{args.downdataset}_{current_time}_{args.gnn_type}_{args.usage_par}_{args.defense_mode}_{args.sim_thre}_{args.attack_type}_{args.makeTE}_{args.poisoned_node}_{args.reg_param_trigger_n}_{args.reg_param_trigger_f}_{args.reg_param_gnn}'
    folder_paths = f'./results/aiolevel_dataset/{args.predataset}_{args.downdataset}_{current_time}_{args.gnn_type}_{args.usage_par}_{args.defense_mode}_{args.sim_thre}_{args.attack_type}_{args.makeTE}_{args.poisoned_node}_{args.reg_param_trigger_n}_{args.reg_param_trigger_f}_{args.reg_param_gnn}/pic_result'
    ...
    if args.downdataset == "Cora":
        down_num_class = 7
        adapt_steps = 20
        down_meta_train_task_id_list = [0, 4, 1, 5, 6]
        down_meta_test_task_id_list = [3, 2]

    elif args.downdataset == "CiteSeer":
        down_num_class = 6
        adapt_steps = 25
        down_meta_train_task_id_list = [0, 1, 2, 3]
        down_meta_test_task_id_list = [4, 5]

    elif args.downdataset == "Computers":
        down_num_class = 10
        adapt_steps = 10
        down_meta_train_task_id_list = [0, 1, 2, 3, 4, 5, 6, 7]
        ...
        if args.gnn_type == 'GAT':
            ...
            adapt_steps = 10
        down_meta_train_task_id_list = [0, 1]
        down_meta_test_task_id_list = [1, 2]

    elif args.downdataset == "Photo":
        down_num_class = 8
        adapt_steps = 10
        down_meta_train_task_id_list = [0, 1, 2, 6, 4, 7]
        down_meta_test_task_id_list = [3, 5]
    else:
        raise ValueError
    ...
    if args.usage_par == 'prompt':
        ...
        prompt_w_h_poison(backdoor_node_features=backdoor_node_feature, makeTE=args.makeTE,
                          ...
                          num_trigger_node=args.num_trigger_node, down_dataname=args.downdataset,
                          pre_dataname=args.predataset, gnn_type=args.gnn_type, pre_num_class=pre_num_class,
                          down_num_class=down_num_class, task_type='multi_class_classification', is_poison=True,
        ...
        backdoor_maml, backdoor_gnn, backdoor_opt, backdoor_lossfn = model_components(is_poison=True,
                                                                                      ...
                                                                                      dataname=args.predataset,
                                                                                      gnn_type=args.gnn_type,
                                                                                      down_dataname=args.downdataset)
        meta_train_maml(train_meta_epoch, backdoor_maml, backdoor_gnn, backdoor_lossfn, backdoor_opt,
                        down_meta_train_task_id_list,
                        args.downdataset, adapt_steps=adapt_steps, K_shot=200)

        meta_test_adam(is_poison=True, meta_test_task_id_list=down_meta_test_task_id_list, dataname=args.downdataset,
                       K_shot=200, seed=seed, maml=backdoor_maml, gnn=backdoor_gnn,
                       adapt_steps_meta_test=adapt_steps, lossfn=backdoor_lossfn,
                       backdoor_node_features=backdoor_node_feature, makeTE=args.makeTE,
