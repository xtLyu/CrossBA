...
class LightPrompt(torch.nn.Module):
    ...
    def token_view(self, ):
        ...
        for i, tokens in enumerate(self.token_list):
            ...
            inner_adj = torch.where(token_sim < self.inner_prune, 0.0, token_sim)
...
class HeavyPrompt(LightPrompt):
    ...
    def forward(self, graph_batch: Batch):
        ...
        for g in Batch.to_data_list(graph_batch):
            ...
            cross_adj = torch.where(cross_sim < self.cross_prune, 0.0, cross_sim)
...
class FrontAndHead(torch.nn.Module):
    def __init__(self, input_dim, hid_dim=16, num_classes=2,task_type="multi_label_classification",
    ...
    def forward(self, graph_batch, gnn):
        prompted_graph = self.PG.forward(graph_batch)
