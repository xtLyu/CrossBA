...
# mnj: multiple trigger nodes link target node
...
def multi_class_NIG_poison_class_aio(dataname, down_class_list, shots=100):
    statistic = defaultdict(list)
...
def multi_class_NIG_clean_class_aio(dataname, down_class_list, shots=100):
    statistic = defaultdict(list)
...
def load_data4pretrain_poison(dataname='CiteSeer', num_parts=200):
...
def load_data4pretrain(dataname='CiteSeer'):
    graph_list = pk.load(open('./Dataset/{}/pretrain_3_hop_neighbors.data'.format(dataname), 'br'))['pos']
    input_dim = len(graph_list[0].x[0])
    hid_dim = input_dim

    return graph_list, input_dim, hid_dim

def load_data4pretrain_class(dataname='CiteSeer'):
    graph_list = pk.load(open('./Dataset/class/{}/pretrain_3_hop_neighbors.data'.format(dataname), 'br'))['pos']
    input_dim = len(graph_list[0].x[0])
    hid_dim = input_dim

    return graph_list, input_dim, hid_dim

def load_data4pretrain_graph(dataname='ENZYMES'):
